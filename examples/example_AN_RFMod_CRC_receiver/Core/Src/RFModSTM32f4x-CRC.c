...
bool envia (const uint8_t* data, uint8_t len)
{
	...
	HAL_CRC_Calculate(&hcrc, (uint32_t)count<<24 ,1);//primeira chamada de CRC
	...
	for(i=0; i<len; i++)
	{
		crc = atualiza_crc_ethernet(data[i]);
	...
	p[index++] = simbolos[(crc >> 16) & 0xf];
	p[index++] = simbolos[(crc >> 20) & 0xf];
	p[index++] = simbolos[(crc >> 24) & 0xf];
	p[index++] = simbolos[(crc >> 28) & 0xf];
...
void validaRxBuf()
{
	...
	aux[0] = (uint32_t)_rxBuf[0] << 24;
	HAL_CRC_Calculate(&hcrc, aux, 1);// primeira chamada é com calculate

	for(uint8_t i = 1; i < _rxBufTam - 4; i++)
		crc = atualiza_crc_ethernet(_rxBuf[i]);//chamadas com accumulate, mas espera o crc ao final da mensagem

	//últimos 32 são o crc
	aux[0] |= (uint32_t) _rxBuf[_rxBufTam - 1] << 24; //MSByte
	aux[0] |= (uint32_t) _rxBuf[_rxBufTam - 2] << 16;
	aux[0] |= (uint32_t) _rxBuf[_rxBufTam - 3] << 8;
	aux[0] |= (uint32_t) _rxBuf[_rxBufTam - 4];
	crc = HAL_CRC_Accumulate(&hcrc, aux, 1 );
...
uint32_t atualiza_crc_ethernet(uint8_t dados)
{
	uint32_t crc;
	uint32_t dados32[1] = { (uint32_t)dados << 24 };

	crc = HAL_CRC_Accumulate(&hcrc, dados32, 1);
